...
type InvokeConfig = {
  configurable?: ConfigurableContext;
};
...
type PermissionState = FetchState & {
  role: UserRole;
};

type FinalState = PermissionState & {
  locale: Locale;
  finalContent: string;
};
...
const fetchUserFromConfig = RunnableLambda.from<string, FetchState>(
  async (input, config) => {
    const userId = (config as InvokeConfig | undefined)?.configurable?.userId;
...
const checkPermissionByRole = RunnableLambda.from<FetchState, PermissionState>(
  async (state, config) => {
    const role =
      (config as InvokeConfig | undefined)?.configurable?.role ?? "普通用户";
...
>(async (state, config) => {
  const locale =
    (config as InvokeConfig | undefined)?.configurable?.locale ?? "zh-CN";
...
const chain = RunnableSequence.from<string, FinalState>([
  fetchUserFromConfig,
