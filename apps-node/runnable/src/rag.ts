...
import { createChatModel, createEmbeddings } from "@ermuz/node-shared";
...
type PromptInput = RetrievalOutput & {
  hasContext: boolean;
  context: string;
};
...
const milvusSearch = RunnableLambda.from<RagQueryInput, RetrievalOutput>(
  async (input) => {
    ...
    try {
      ...
      const results = (searchResult.results ?? []) as MilvusSearchResultItem[];
...
const buildPromptInput = RunnableLambda.from<RetrievalOutput, PromptInput>(
  async (input) => {
...
const ragChain = RunnableSequence.from<RagQueryInput, string>([
  milvusSearch,
  buildPromptInput,
  RunnableLambda.from<PromptInput, GateOutput>(async (input) => {
